<?php
require_once('corelib/dataaccess_lite.php');
/*
#tablePrefix yacrs_
#database sqlite
...
{
	$query = "CREATE TABLE yacrs_lticonsumer(id INTEGER PRIMARY KEY, keyHash VARCHAR(40), consumer_key VARCHAR(255), name VARCHAR(80), secret VARCHAR(255));";
	dataConnection::runQuery($query);
	$query = "CREATE TABLE yacrs_session(id INTEGER PRIMARY KEY, ownerID VARCHAR(35), title VARCHAR(80), created DATETIME, questions TEXT, currentQuestion INTEGER, questionMode INTEGER, endtime DATETIME, sessionstarttime DATETIME, sessionOpen INTEGER, activeSubsession_id INTEGER, sessionendtime DATETIME, visible INTEGER, allowGuests INTEGER, multiSession INTEGER, ublogRoom INTEGER, maxMessagelength INTEGER, allowQuReview INTEGER, allowTeacherQu INTEGER, courseIdentifier VARCHAR(20), defaultQuActiveSecs INTEGER, extras TEXT);";
	dataConnection::runQuery($query);
	$query = "CREATE TABLE yacrs_subsession(id INTEGER PRIMARY KEY, session_id INTEGER, title VARCHAR(80), starttime DATETIME, endtime DATETIME);";
	dataConnection::runQuery($query);
	$query = "CREATE TABLE yacrs_ltisessionlink(id INTEGER PRIMARY KEY, client_id INTEGER, resource_link_id VARCHAR(255), session_id INTEGER);";
	dataConnection::runQuery($query);
	$query = "CREATE TABLE yacrs_userInfo(id INTEGER PRIMARY KEY, username VARCHAR(80), name VARCHAR(45), email VARCHAR(85), nickname VARCHAR(45), phone VARCHAR(20), sessionCreator INTEGER, isAdmin INTEGER, teacherPrefs TEXT);";
	dataConnection::runQuery($query);
	$query = "CREATE TABLE yacrs_question(id INTEGER PRIMARY KEY, ownerID VARCHAR(35), session_id INTEGER, title VARCHAR(80), definition TEXT, responsetype VARCHAR(20), multiuse INTEGER);";
	dataConnection::runQuery($query);
	$query = "CREATE TABLE yacrs_systemQuestionLookup(id INTEGER PRIMARY KEY, qu_id INTEGER, name VARCHAR(10));";
	dataConnection::runQuery($query);
	$query = "CREATE TABLE yacrs_questionInstance(id INTEGER PRIMARY KEY, title VARCHAR(80), theQuestion_id INTEGER, inSession_id INTEGER, subsession_id INTEGER, starttime DATETIME, endtime DATETIME, screenshot VARCHAR(60), extras TEXT);";
	dataConnection::runQuery($query);
	$query = "CREATE TABLE yacrs_sessionMember(id INTEGER PRIMARY KEY, session_id INTEGER, userID VARCHAR(35), name VARCHAR(45), nickname VARCHAR(45), email VARCHAR(85), user_id INTEGER, joined DATETIME, lastresponse DATETIME, mobile VARCHAR(20));";
	dataConnection::runQuery($query);
	$query = "CREATE TABLE yacrs_response(id INTEGER PRIMARY KEY, user_id INTEGER, question_id INTEGER, value TEXT, isPartial INTEGER, time DATETIME);";
	dataConnection::runQuery($query);
	$query = "CREATE TABLE yacrs_message(id INTEGER PRIMARY KEY, user_id INTEGER, session_id INTEGER, subsession_id INTEGER, isTeacherQu INTEGER, private INTEGER, posted DATETIME, message TEXT, replyTo_id INTEGER);";
	dataConnection::runQuery($query);
    $query = "CREATE TABLE yacrs_message_tag_link(message_id INTEGER, tag_id INTEGER);";
	dataConnection::runQuery($query);
	$query = "CREATE TABLE yacrs_tag(id INTEGER PRIMARY KEY, text VARCHAR(20), session_id INTEGER);";
	dataConnection::runQuery($query);
}
...
class lticonsumer
{
	...
	function insert()
	{
		...
		$result = dataConnection::runQuery($query);
		$result2 = dataConnection::runQuery("SELECT last_insert_rowid() AS id;");
